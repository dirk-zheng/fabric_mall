...
export default function AdminUsers({ manageRoles = false }) {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((user) => !value || `${user.name} ${user.account} ${user.role} ${(user.visitorIds || []).join(' ')}`.toLowerCase().includes(value));
  }, [users, query]);
      ...
      <main className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <label className="relative mb-6 block max-w-xl">
          ...
          <input
            ...
            placeholder="Search name, account, role or visitor ID"
