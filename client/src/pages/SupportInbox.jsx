...
export default function SupportInbox() {
  ...
  useEffect(() => {
    const offConversation = wsClient.on('support.conversation.updated', (updated) => {
      setConversations((current) => {
        ...
        if (updated.status !== 'closed' && (admin || updated.status === 'waiting_human' || updated.assignedTo === user.account)) next.unshift(updated);
  ...
  }, [selected?.id, user.account, admin]);
  ...
  const filtered = useMemo(() => conversations.filter((conversation) => {
    ...
    if (filter === 'mine') return conversation.assignedTo === user.account;
    if (filter === 'resolved') return conversation.status === 'resolved';
    return conversation.status !== 'resolved';
  }), [conversations, filter, user.account]);

  const canReply = selected?.assignedTo === user.account && selected?.status === 'human_active';
  const canClaim = selected && selected.status !== 'resolved' && selected.assignedTo !== user.account;
          ...
          <main className="flex min-h-[520px] flex-col">
            {!selected ? <div className="flex flex-1 flex-col items-center justify-center px-8 text-center text-slate-400"><Headphones size={42} className="mb-4 text-slate-300" /><p className="font-semibold text-slate-600">Select a conversation</p><p className="mt-1 text-sm">Waiting conversations can be claimed by one representative at a time.</p></div> : <>
              <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
                <div><h2 className="font-semibold text-slate-900">{selected.customerName}</h2><p className="text-xs text-slate-500">@{selected.customerAccount} · {statusLabels[selected.status]}</p></div>
